...
def normalize_protocol_for_marzban(protocol: str) -> str:
    ...
    return MARZBAN_PROTOCOL_NAMES.get(proto_lower, proto_lower)
    ...
    async def create_user(
        ...
        *,
        ...
        try:
            ...
            elif normalized_protocol == "vmess":
                # VMess doesn't use flow, alterId is deprecated (use 0)
                proxies["vmess"] = {}
            elif normalized_protocol == "trojan":
                # Trojan uses password (auto-generated by Marzban)
                proxies["trojan"] = {}
            elif normalized_protocol in {"shadowsocks", "ss"}:
                # Shadowsocks needs encryption method (Marzban uses defaults)
                proxies["shadowsocks"] = {}
            else:
                # Unknown protocol - log warning and try anyway
                logger.warning(
                    f"Unknown protocol '{protocol}' for Marzban user creation. "
                    "Enabling all protocols as fallback."
                )
                proxies = {
                    "vless": {"flow": flow or "xtls-rprx-vision"},
                    "vmess": {},
                    "trojan": {},
                    "shadowsocks": {},
                }
