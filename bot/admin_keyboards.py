...
def admin_payments_list_keyboard(
    payments: list[Payment], page: int = 0, per_page: int = 5
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton("⬅️ قبلی", callback_data=f"admin_payments_pending_page_{page - 1}")
        )
    if end < len(payments):
        nav_buttons.append(
            InlineKeyboardButton("بعدی ➡️", callback_data=f"admin_payments_pending_page_{page + 1}")
        )
    if nav_buttons:
...
def admin_payment_detail_keyboard(payment_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ تایید پرداخت", callback_data=f"admin_payment_approve_{payment_id}"
                ),
                InlineKeyboardButton(
                    "❌ رد پرداخت", callback_data=f"admin_payment_reject_{payment_id}"
                ),
            ],
            [
                InlineKeyboardButton(
                    "🔙 بازگشت به لیست پرداخت‌ها", callback_data="admin_payments_pending"
                )
            ],
        ]
    )
...
def admin_users_list_keyboard(
    users: list[User], page: int = 0, per_page: int = 10
) -> InlineKeyboardMarkup:
    ...
    for u in page_users:
        ...
        keyboard.append([InlineKeyboardButton(text, callback_data=f"admin_user_detail_{u.id}")])

    # Pagination
    nav_buttons: list[InlineKeyboardButton] = []
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton("⬅️ قبلی", callback_data=f"admin_users_page_{page - 1}")
        )
    if end < len(users):
        nav_buttons.append(
            InlineKeyboardButton("➡️ بعدی", callback_data=f"admin_users_page_{page + 1}")
        )
    if nav_buttons:
        keyboard.append(nav_buttons)
...
def admin_services_list_keyboard(
    services: list[Service], page: int = 0, per_page: int = 10
) -> InlineKeyboardMarkup:
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton("⬅️ قبلی", callback_data=f"admin_services_page_{page - 1}")
        )
    if end < len(services):
        nav_buttons.append(
            InlineKeyboardButton("➡️ بعدی", callback_data=f"admin_services_page_{page + 1}")
        )
    if nav_buttons:
        keyboard.append(nav_buttons)
...
def admin_service_detail_keyboard(service_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "📈 افزودن ترافیک", callback_data=f"admin_service_addgb_{service_id}"
                ),
                InlineKeyboardButton(
                    "🔐 Rotate", callback_data=f"admin_service_rotate_{service_id}"
                ),
            ],
            [
                InlineKeyboardButton(
                    "❌ حذف سرویس", callback_data=f"admin_service_remove_{service_id}"
                )
            ],
...
def admin_panels_list_keyboard(
    panels: list[Panel], page: int = 0, per_page: int = 10
) -> InlineKeyboardMarkup:
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton("⬅️ قبلی", callback_data=f"admin_panels_page_{page - 1}")
        )
    if end < len(panels):
        nav_buttons.append(
            InlineKeyboardButton("➡️ بعدی", callback_data=f"admin_panels_page_{page + 1}")
        )
    if nav_buttons:
        keyboard.append(nav_buttons)
