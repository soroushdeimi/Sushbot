...
def main_menu_keyboard(user: User | None = None) -> InlineKeyboardMarkup:
    ...
    if is_enabled("purchase"):
        keyboard.append([InlineKeyboardButton(t("purchase_service", user, lang), callback_data="purchase")])
    if is_enabled("services"):
        keyboard.append([InlineKeyboardButton(t("my_services", user, lang), callback_data="my_services")])
...
def wallet_menu_keyboard(user: User | None = None) -> InlineKeyboardMarkup:
    ...
    lang = get_user_language(user)
    keyboard = [
        [InlineKeyboardButton("💰 " + t("wallet_balance", user, lang), callback_data="wallet_balance")],
...
def support_keyboard(user: User | None = None) -> InlineKeyboardMarkup:
    ...
    lang = get_user_language(user)
    keyboard = [
        [InlineKeyboardButton("📝 " + t("create_ticket", user, lang), callback_data="create_ticket")],
...
def product_detail_keyboard(product_id: int, user: User | None = None) -> InlineKeyboardMarkup:
    ...
    lang = get_user_language(user)
    keyboard = [
        [InlineKeyboardButton("✅ " + t("purchase", user, lang), callback_data=f"confirm_purchase_{product_id}")],
        [InlineKeyboardButton("🎁 " + t("enter_discount_code", user, lang), callback_data=f"discount_enter_{product_id}")],
        [InlineKeyboardButton(t("back", user, lang), callback_data="purchase")],
    ]
    return InlineKeyboardMarkup(keyboard)


def purchase_with_discount_keyboard(product_id: int, discount_code: str | None = None, user: User | None = None) -> InlineKeyboardMarkup:
    """Purchase confirmation keyboard with applied discount."""
    lang = get_user_language(user)
    keyboard = [
        [InlineKeyboardButton("✅ " + t("proceed_to_payment", user, lang), callback_data=f"purchase_pay_{product_id}_{discount_code or ''}")],
...
def payment_gateway_keyboard(purchase_id: str, user: User | None = None) -> InlineKeyboardMarkup:
    ...
    if settings.card_to_card_enabled:
        keyboard.append([InlineKeyboardButton("💳 " + t("card_to_card", user, lang), callback_data=f"payment_card_{purchase_id}")])
    if settings.nowpayments_enabled:
        keyboard.append([InlineKeyboardButton("🌐 " + t("nowpayments", user, lang), callback_data=f"payment_nowpay_{purchase_id}")])
    if settings.aqayepardakht_enabled:
        keyboard.append([InlineKeyboardButton("🏦 " + t("aqayepardakht", user, lang), callback_data=f"payment_aqaye_{purchase_id}")])

    keyboard.append([InlineKeyboardButton(t("cancel", user, lang), callback_data=f"cancel_order_{purchase_id}")])
    return InlineKeyboardMarkup(keyboard)
...
def service_manage_keyboard(service_id: int, user: User | None = None) -> InlineKeyboardMarkup:
    ...
    lang = get_user_language(user)
    keyboard = [
        [InlineKeyboardButton("📋 " + t("get_config", user, lang), callback_data=f"service_config_{service_id}")],
        [InlineKeyboardButton("🔗 " + t("get_subscription", user, lang), callback_data=f"service_sub_{service_id}")],
        [InlineKeyboardButton("🔄 " + t("rotate", user, lang), callback_data=f"service_rotate_{service_id}")],
        [InlineKeyboardButton(t("back", user, lang), callback_data="my_services")],
    ]
    return InlineKeyboardMarkup(keyboard)


def confirm_action_keyboard(action: str, item_id: int, user: User | None = None) -> InlineKeyboardMarkup:
    """Confirmation keyboard for actions."""
    lang = get_user_language(user)
    keyboard = [
        [
            InlineKeyboardButton("✅ " + t("confirm", user, lang), callback_data=f"{action}_confirm_{item_id}"),
            InlineKeyboardButton("❌ " + t("cancel", user, lang), callback_data=f"{action}_cancel_{item_id}"),
        ]
    ]
    return InlineKeyboardMarkup(keyboard)


def service_products_keyboard(service_id: int, products: list, user: User | None = None) -> InlineKeyboardMarkup:
    ...
    for product in products:
        ...
        keyboard.append([InlineKeyboardButton(button_text, callback_data=f"service_product_{service_id}_{product.id}")])
    keyboard.append([InlineKeyboardButton(t("back", user, lang), callback_data=f"service_{service_id}")])
    return InlineKeyboardMarkup(keyboard)
