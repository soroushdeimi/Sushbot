...
def upgrade() -> None:
    op.execute(
        "ALTER TABLE services ADD COLUMN IF NOT EXISTS last_expiry_notified_at TIMESTAMPTZ NULL;"
    )
    op.execute(
        "ALTER TABLE services ADD COLUMN IF NOT EXISTS last_traffic_notified_at TIMESTAMPTZ NULL;"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_services_last_expiry_notified_at ON services(last_expiry_notified_at);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_services_last_traffic_notified_at ON services(last_traffic_notified_at);"
    )
